let elem =
  '<div class = "row" > <div class= "checklist" ><input type = "checkbox" name = "check" class = "check btn" value = "finished"><input type = "text" class = "text"> <div class = "open-button"></div></div> <button type = "button" class = "remove-button hide">x</button></div>';
...
function addListener() {
  ...
  deleteRow.addEventListener("click", () => {
    var nodes = Array.prototype.slice.call(
      document.getElementById("env").children[0].children
    );
  ...
  currentRow.addEventListener("mouseleave", () => {
    ...
    if (!deleteRow.classList.contains("hide")) deleteRow.classList.add("hide");
  });
  ...
  box.addEventListener("click", () => {
    ...
    if (checkArr[index]) textbox.classList.add("strike-through");
